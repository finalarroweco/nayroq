...
export default function GettingStarted(){const{lang,t}=useLanguage();
 ...
 return <main className="appshell"><AppNav/><div className="appbody"><main className="module"><header><Link href="/dashboard">← Dashboard</Link><div className="logo"><i>N</i><b>NAYROQ</b></div><span>{lang==="ar"?"البدء":"Getting Started"}</span><LanguageSwitch/></header><section><div className="eyebrow">{lang==="ar"?"الإعداد":"SETUP"}</div><h1>{t("launch")}</h1><p>{lang==="ar"?`${done} من ${steps.length} خطوات مكتملة`:`${done} of ${steps.length} steps completed`}</p><div className="setupbar"><i style={{width:(done/steps.length*100)+"%"}}/></div><div className="setupsteps">{steps.map((s,i)=><Link href={s[2]} className={s[1]?"done":""} key={s[0]}><i>{s[1]?"✓":i+1}</i><div><b>{s[0]}</b><p>{s[3]}</p></div><span>{s[1]?t("complete"):t("continueArrow")+" →"}</span></Link>)}</div></section></main>
}