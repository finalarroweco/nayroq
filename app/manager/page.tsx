  ...
  return <main className="appshell"><AppNav/><div className="appbody"><main className="module">
    ...
    <section>
      ...
      <div className="managergrid">
        ...
        <div className="panel"><h3>{t("attention")} <em>{data.handoffs.length}</em></h3>{data.handoffs.length?data.handoffs.slice(0,6).map((x:any)=><div className="attention" key={x.id}><div><b>{x.contacts?.name||x.contacts?.phone||"Customer"}</b><small>{x.channel} · AI: {x.ai_employees?.name||"—"}</small></div><Link href="/inbox">Open →</Link></div>):<div className="placeholder">{t("noAttention")}</div>}</div>
        <div className="panel"><h3>{t("openOpportunities")}</h3>{data.hot.length?data.hot.map((x:any)=><div className="attention" key={x.id}><div><b>{x.contacts?.name||x.contacts?.phone||"Lead"}</b><small>{x.stage} · {x.value||0} {x.currency}</small></div><Link href={"/leads/"+x.id}>Review →</Link></div>):<div className="placeholder">{t("noOpportunities")}</div>}</div>
