...
export default function Manager(){const{lang,t}=useLanguage();
  ...
  return <main className="appshell"><AppNav/><div className="appbody"><main className="module">
    ...
    <section>
      ...
      <p>{lang==="ar"?data.team.filter((x:any)=>x.status==="active").length+" موظف AI نشط · "+data.followups.length+" متابعة مجدولة · "+data.handoffs.length+" محادثة تحتاج تدخلاً بشرياً":data.team.filter((x:any)=>x.status==="active").length+" active AI employees · "+data.followups.length+" follow-ups scheduled · "+data.handoffs.length+" conversations need human attention"}</p>
      <div className="managerstats">{(lang==="ar"?[["المحادثات",data.conversations],["العملاء المحتملون",data.leads],["المؤهلون",data.qualified],["عروض الأسعار",data.quotes],["الناجحة",data.wins],["الإيرادات",data.revenue+" "+data.company.currency]]:[["Conversations",data.conversations],["Leads",data.leads],["Qualified",data.qualified],["Quotations",data.quotes],["Won",data.wins],["Revenue",data.revenue+" "+data.company.currency]]).map((x:any)=><div className="stat" key={x[0]}><span>{x[0]}</span><strong>{x[1]}</strong></div>)}</div>
      <div className="managergrid">
        ...
        <div className="panel"><h3>{t("attention")} <em>{data.handoffs.length}</em></h3>{data.handoffs.length?data.handoffs.slice(0,6).map((x:any)=><div className="attention" key={x.id}><div><b>{x.contacts?.name||x.contacts?.phone||(lang==="ar"?"عميل":"Customer")}</b><small>{x.channel} · {lang==="ar"?"موظف AI: ":"AI: "}{x.ai_employees?.name||"—"}</small></div><Link href="/inbox">{lang==="ar"?"فتح ←":"Open →"}</Link></div>):<div className="placeholder">{t("noAttention")}</div>}</div>
        <div className="panel"><h3>{t("openOpportunities")}</h3>{data.hot.length?data.hot.map((x:any)=><div className="attention" key={x.id}><div><b>{x.contacts?.name||x.contacts?.phone||(lang==="ar"?"عميل محتمل":"Lead")}</b><small>{x.stage} · {x.value||0} {x.currency}</small></div><Link href={"/leads/"+x.id}>{lang==="ar"?"مراجعة ←":"Review →"}</Link></div>):<div className="placeholder">{t("noOpportunities")}</div>}</div>
